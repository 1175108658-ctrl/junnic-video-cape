...
- Repeat the complete reference-image binding and active scene continuity at the start of every independent block. Use exactly the number of supplied reference images that the source and user setup require; never add a third reference merely because a background or supporting role appears. The canonical mapping format is `参考图一对应男主A` and `参考图二对应女主B`: keep the image ordinal (`参考图一/二`) and the role label (`男主A/女主B`) together in every independent block. If the user supplies different role labels or more references, preserve the user's exact mapping and repeat each image-to-role pair literally. Do not collapse the mapping to `参考图A/参考图B`, `女方/男方`, or other unlabeled positional wording, and do not silently swap image order. Keep source dialogue names and relationships as observed, and do not invent names from prior rounds.
